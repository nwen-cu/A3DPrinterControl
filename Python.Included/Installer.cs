...
namespace Python.Included
{
    public static class Installer
    {
        ...
        public static void PipInstallModule(string module_name, bool force = false, bool runInBackground = true)
        {
            if (!IsPipInstalled())
                try { InstallPip(); } catch { throw new FileNotFoundException("pip is not installed"); }
        ...
        public static void InstallPip(bool force = false, bool runInBackground = true)
        {
            if (IsPipInstalled() && !force) return;
            string libDir = Path.Combine(EmbeddedPythonHome, "Lib");
            if (!Directory.Exists(libDir))
                Directory.CreateDirectory(libDir);
            string drive = RuntimeInformation.IsOSPlatform(OSPlatform.Windows) ? "C: &&" : "";
            RunCommand($"{drive}cd {libDir} && curl https://bootstrap.pypa.io/get-pip.py -o get-pip.py", runInBackground);
            RunCommand($"{drive}cd {EmbeddedPythonHome} && python.exe Lib\\get-pip.py", runInBackground);
        }
        ...
        public static bool IsModuleInstalled(string module)
        {
            ...
            string siteDir = Path.Combine(EmbeddedPythonHome, "Lib", "site-packages", module);
            return (Directory.Exists(moduleDir) && File.Exists(Path.Combine(moduleDir, "__init__.py"))
                || Directory.Exists(siteDir) && File.Exists(Path.Combine(siteDir, "__init__.py")));
        }

...
def OnGeneratingInfill():
	...
	for shape in shape_objects:
		...
		if shape_points[0] != shape_points[-1]:
			infills.append([])
			continue
		density = shape.Command.InfillOption.Density
		sp = sg.LineString(shape_points)
		boundBox = find_shape_box(sp)
		vertInfillVectors = vert_infill(sp, boundBox, density)
		infills.append(vertInfillVectors)

	return infills

def pointlistConverter(pointlist):
	tuplelist = []
...
def vert_infill(shp, box, density = 1):
	vecs = [[[],[]]]
	i = box[0]
	while (i <= box[2]):
...
def horiz_infill(shp, box, density = 1):
	vecs = [[[],[]]]
	i = box[1]
	while (i <= box[3]):
